        ...
        <div className="hidden lg:flex items-center gap-4">
          {headerData.map((item) => (
            <ul key={item.id} className="relative py-2 group">
              <li className="px-0 xl:px-1 font-titleFont text-[13px] xl:text-[14px] text-[#004165] font-semibold bg-clip-text hover:bg-gradient-to-r from hover:bg-pink-400 to uppercase group cursor-pointer hover:text-[#772432] duration-300">
                {item.title}
              </li>
              <span className="absolute w-full transform scale-0 group-hover:scale-100 duration-500 h-[3px] bottom-0 bg-gradient-to-r from-[#004165] via-[#772432] to-[#772432] inline-block"></span>
            </ul>
          ))}
        </div>
